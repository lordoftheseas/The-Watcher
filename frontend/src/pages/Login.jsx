...
function Login() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (error) {
        // Better error message for unsupported provider
  ...
  const handleGitHubLogin = async () => {
    try {
      ...
      if (error) {
        // Better error message for unsupported provider
        if (error.message.includes('provider is not enabled') || error.message.includes('Unsupported provider')) {
          setError('GitHub sign-in is not configured yet. Please use email/password or contact support.')
        } else {
          setError(error.message)
        }
      }
    } catch (err) {
      setError('GitHub sign-in is not available. Please use email/password.')
    ...
    <div className="auth-container">
      <div className="auth-card">
        <div className="auth-header">
          ...
          <p className="auth-subtitle">Sign in to your account to continue monitoring</p>
        </div>
        ...
        {oauthEnabled && (
          <>
            ...
            <div className="social-login">
              <button className="btn-social" onClick={handleGoogleLogin} type="button" disabled={loading}>
              ...
              <button className="btn-social" onClick={handleGitHubLogin} type="button" disabled={loading}>
                <svg viewBox="0 0 24 24" width="20" height="20">
